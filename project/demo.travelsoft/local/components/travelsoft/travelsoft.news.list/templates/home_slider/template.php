<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();
   ...
   ?>
<div class="main_slider-v3--container">
    <div class="owl-carousel owl-theme js-main_slider h-100">
        <?foreach($arResult["ITEMS"] as $arItem):?>
        <?
        $this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
        $this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
        ?>
            <? $imagesDesctop = getSrc($arItem['DISPLAY_PROPERTIES']['PICTURES']['VALUE'], ['width' => 2120, 'height' => 1380], NO_PHOTO_PATH_1920_600, 1) ?>
            <? $imagesTablet = getSrc($arItem['DISPLAY_PROPERTIES']['PICTURES']['VALUE'], ['width' => 1024, 'height' => 768], NO_PHOTO_PATH_1920_600, 1) ?>
            <? $imagesMobile = getSrc($arItem['DISPLAY_PROPERTIES']['PICTURES']['VALUE'], ['width' => 414, 'height' => 736], NO_PHOTO_PATH_1920_600, 1) ?>
            <div class="main_slider-v3--item">
            <div class="main_slider-v3--item-img">
                <picture>
                    <source class="owl-lazy" media="(min-width: 1320px)" data-srcset="<?= $imagesDesctop[0] ?>" />
                    <source class="owl-lazy" media="(min-width: 840px)" data-srcset="<?= $imagesTablet[0] ?>" />
                    <source class="owl-lazy" media="(min-width: 0px)" data-srcset="<?= $imagesMobile[0] ?>" />
                    <img class="card-img-top owl-lazy" data-src="<?= $imagesMobile[0] ?>" src="<?= $imagesMobile[0] ?>" alt="slide" />
                </picture>
            </div>
        </div>
        <?endforeach;?>
...
<!--                </div>-->
<!--				--><?//if(!empty($arItem["PROPERTIES"]["LINK"]["VALUE"])):?><!--<a class="btn _tt-uc _mt-20 btn-white btn-ghost btn-lg" href="--><?//=$arItem["PROPERTIES"]["LINK"]["VALUE"]?><!--">--><?//= GetMessage("MORE") ?><!--</a>--><?//endif;?>
<!--              </div>-->
<!--            </div>-->
<!--          </div>-->
<!--        </div>-->
